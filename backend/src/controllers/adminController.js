...
//   }
// };
// --- Quản Lý Yêu Cầu Hoàn Trả (Return Request Management) ---

/**
 * @desc Lấy tất cả yêu cầu hoàn trả
 * @route GET /api/admin/return-requests
 * @access Riêng tư (Admin)
 */
exports.getAllReturnRequestsAdmin = async (req, res) => {
  try {
    ...
    const requests = await ReturnRequest.find(query)
      .populate({
        ...
        populate: [
          ...
          { path: "orderId", select: "orderNumber totalPrice status" },
        ],
      })
    ...
    handleError(res, error, "Lỗi khi lấy danh sách yêu cầu hoàn trả");
  }
};

/**
 * @desc Duyệt yêu cầu hoàn trả
 * @route PUT /api/admin/return-requests/:id/approve
 * @access Riêng tư (Admin)
 */
exports.approveReturnRequestAdmin = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (!request) {
      return res
        .status(404)
        .json({ success: false, message: "Không tìm thấy yêu cầu hoàn trả" });
    }
    ...
    if (request.orderItemId?.orderId) {
      await Order.findByIdAndUpdate(request.orderItemId.orderId, {
        status: "rejected", // Hoặc "refunded" tùy theo enum bạn có
      });
    }

    // Gửi email thông báo cho người mua (nếu cần)
    try {
      await sendEmail(
        request.userId.email,
        "Yêu cầu hoàn trả đã được chấp thuận",
        `Kính gửi ${request.userId.username},\n\nYêu cầu hoàn trả của bạn đã được quản trị viên duyệt. Chúng tôi sẽ xử lý việc hoàn tiền sớm nhất.\n\nTrân trọng,\nĐội ngũ Shopii`
      );
    } catch (e) {
      console.warn("Không thể gửi email xác nhận:", e.message);
    }
    ...
    handleError(res, error, "Lỗi khi duyệt yêu cầu hoàn trả");
  }
};

/**
 * @desc Từ chối yêu cầu hoàn trả
 * @route PUT /api/admin/return-requests/:id/reject
 * @access Riêng tư (Admin)
 */
exports.rejectReturnRequestAdmin = async (req, res) => {
  try {
    const { id } = req.params;
    const request = await ReturnRequest.findById(id)
      .populate("userId", "username email");
    ...
    try {
      await sendEmail(
        ...
        `Kính gửi ${request.userId.username},\n\nRất tiếc, yêu cầu hoàn trả của bạn đã bị từ chối. Vui lòng liên hệ với bộ phận hỗ trợ để biết thêm chi tiết.\n\nTrân trọng,\nĐội ngũ Shopii`
      );
    } catch (e) {
      console.warn("Không thể gửi email từ chối:", e.message);
    }

    res.status(200).json({
      success: true,
      message: "Đã từ chối yêu cầu hoàn trả thành công",
      data: request,
    });
  } catch (error) {
